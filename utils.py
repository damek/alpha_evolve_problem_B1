...
def prox_linear_method(h_params, loss_fn,print_every, max_iter=1000, prox_linear_params={}, history = {}):
    ...
    if 'gamma_prox' not in prox_linear_params:
        prox_linear_params = {'abstol': 1e-16, 'reltol': 1e-16, 'feastol': 1e-16, 'max_iters': 2000, 'gamma_prox': gamma_prox}

    solver_params = {key: prox_linear_params[key] for key in ['abstol', 'reltol', 'feastol', 'max_iters', 'verbose']}
    ...
    for i in range(max_iter):
        ...
        if i % print_every == 0:
            print(f"Iteration {i}: loss = {loss.item():.7f}, step_length = {step_length:.7f}, grad_norm = {history['grad_norm_history'][-1]:.7f}, min_loss_found = {history['min_loss_found']:.6f}")
