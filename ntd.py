...
class NTD(Optimizer):
    ...
    def __init__(self,
                 ...
                 s_scale_factor = 1e-6,
                 verbose=False, 
                 grad_tol_break=1e-20,
                 max_grid_size=None,
                 min_goldstein_iters = None):
        ...
        self.grad_tol_break = grad_tol_break
        self.min_goldstein_iters = min_goldstein_iters
    ...
    def _NDescent(self, obj_func, x, g, loss, sigma, T):
        ...
        for i in range(T):
            ...
            if nrmg <= self.grad_tol_break:
                print("small gradient, breaking.", nrmg)
                break
            f_new, g_xp = obj_func(x, -sigma, g/nrmg)
            num_oracle += 1
            if f_new < min_value:
                best_g = g
                min_value = f_new
            if f_new - loss + sigma * nrmg / 8 <= 0:
            # if np.log(f_new/(loss - sigma * nrmg / 8)) <= 0:
                best_g = g
                break
            t = np.random.uniform()
    ...
    def _TDescent(self, obj_func, x, g, loss, sigma, T):
        ...
        for i in range(T):
            ...
            if nrmg <= self.grad_tol_break:
                print("small gradient, breaking.", nrmg)
                break
            f_new, hatg = obj_func(x, -sigma, g/nrmg)
            if f_new < min_value:
                best_g = g
                min_value = f_new
            num_oracle += 1
            if f_new - loss + sigma * nrmg / 8 <= 0:
                # a version with log to avoid overflow
            # if np.log(f_new/(loss - sigma * nrmg / 8)) <= 0:
                best_g = g
                break
            g = self._optimal_average(g, hatg)
    ...
    def _linesearch(self, obj_func, x, g, loss, G, T):
        ...
        while i < G and ((sigma_multiplier * min_sub_norm/s >= sigma) or not self.use_trust_region):
            ...
            if nrmv <= self.grad_tol_break:
                print("small v, breaking", nrmv)
                break
            f_new, hatg = obj_func(x, -sigma, v/nrmv)
            if self.verbose == True:
                print("sigma: " + str(sigma)
                      ...
                      + " TD: " + str(no2))
            num_oracle += 1
            if min_sub_norm <= self.grad_tol_break:
                print("small min_sub_norm, breaking", min_sub_norm)
                break
            if f_new < min_value:
                min_value = f_new
                best_g = v
    ...
    def step(self, closure):
        ...
        if self.min_goldstein_iters == None:
            T = K
        else:
            T = max(self.min_goldstein_iters, K)
        ...
        if self.verbose == True:
            ...
            print("Stationary Measure: " + str(self.stationary_measure[-1]))
